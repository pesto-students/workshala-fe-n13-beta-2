...
import { Paid, Group, Work, School, Upload } from "@mui/icons-material";
import {
  Avatar,
  Button,
  Divider,
...
export const UpdateApplyjobData = (data) => {
  ...
  tempData.forEach(function (k, i) {
    JobDetailsData[i] = {
      ...
      value: data[tempData[i]],
    };
  });

  JobApplnData = {
    companyLogo: companyLogo,
    name: data.companyRef.name,
    title: data.title,
    desc: data.desc,
    type: data.empType,
              ...
              >
                <Grid>
                  ...
                  <Typography style={{ fontSize: 10 }}>
                    uploaded on{" "}
                    {moment(props.data.updatedAt).format("YYYY-MM-DD")}
                  </Typography>
                </Grid>
              </Button>
            </Grid>
...
export default function ApplicationForm() {
  ...
  if (isEmpty(JobApplnData)) return <Loader />;
  else
    return (
